...
To collect code coverage information, run CMake with the `-DCSD_ENABLE_COVERAGE=1` option.
...
```bash
cmake -B build -DCSD_BUILD_DOCS=ON
...
# run standalone
./build/Csd --help
# build docs (requires -DCSD_BUILD_DOCS=ON at configure time)
cmake --build build --target GenerateDocs
```

### Static analysis

clang-tidy can be enabled by configuring CMake with `-DCSD_ENABLE_CLANG_TIDY=ON` and building the `clang-tidy` target.
It analyzes the public headers using the checks configured in `.clang-tidy`.

```bash
cmake -B build -DCSD_ENABLE_CLANG_TIDY=ON
